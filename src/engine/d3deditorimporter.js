...
export async function handleImportFile(file, destDir) {
	...
	const name = file.overrideName ?? file.name;
	...
	switch (ext) {
		...
		case 'gltf': {
			...
			// --- create <base>.<ext>container/ folder
			const folderLabel = `${base}.${srcExt}container`;
			const folderNoSlash = overwrite
				? `${destDir}/${folderLabel}`
				: uniqueFilePath(zip, destDir, folderLabel);
			...
			// ======================================
			{
				...
				// subfolders
				const matsDir = norm(overwrite ? `${virtualDir}materials/` : uniqueFilePath(zip, virtualDir, 'materials/'));
				const texDir  = norm(overwrite ? `${virtualDir}textures/`  : uniqueFilePath(zip, virtualDir, 'textures/'));
				zip.folder(matsDir);
				zip.folder(texDir);
				...
				const bySig = new Map();  // signature -> outRel (.mat path)
				...
				const writeTexture = async (tex, baseHint) => {
					...
					try {
						...
						const outRel = norm(
							overwrite
								? `${texDir}${base}.png`
								: uniqueFilePath(zip, texDir, `${base}.png`)
						);
						...
						return _root.resolveAssetId(outRel) || null;
					} catch {
						return null;
					}
				};
				...
				for (const mat of materialSet) {
					...
					outRel = norm(
						overwrite
							? `${matsDir}${baseName}.mat`
							: uniqueFilePath(zip, matsDir, `${baseName}.mat`)
					);
				...
				if (materialFiles.length) {
					...
					const manifest = { count: files.length, files, byName: byNameUUID };
		...
		default: {
			const outRel = overwrite ? `${destDir}/${name}` : uniqueFilePath(zip, destDir, name);
			zip.file(outRel, buf);
			return { wrote: [outRel] };
		}
	}
