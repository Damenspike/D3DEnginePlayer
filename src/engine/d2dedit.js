...
export default class D2DEdit {
	...
	render() {
		...
		this.curveDragPreservePoint = !_input.getKeyDown('shift');
	...
	_onMouseMove(e) {
		...
		if(this.dragging && this.dragObj) {
			...
			const drawer = this.d2drenderer?.drawer;
			const snappingOn = !!_editor.draw2d?.snapToPoints && !!drawer && (!this._curveDrag?.active || this.curveDragPreservePoint);
