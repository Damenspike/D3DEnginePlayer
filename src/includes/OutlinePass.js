...
class OutlinePass extends Pass {
	...
	constructor( resolution, scene, camera, selectedObjects ) {
		...
		// combine (base + outline) material
		this.combineUniforms = {
			'tDiffuse':      { value: null },                        // base (readBuffer)
		...
		this.materialCombine = new ShaderMaterial({
			...
			fragmentShader: `
				...
				uniform sampler2D tDiffuse;      // base color (readBuffer)
				...
				void main() {
					...
					// same outline build logic as overlayMaterial, but done in-shader
					float visibilityFactor = (1.0 - maskColor.g) > 0.0 ? 1.0 : 0.5;
					vec4 edgeValue = edgeValue1 + edgeValue2 * edgeGlow;
					vec4 outline   = edgeStrength * maskColor.r * edgeValue;
		
					if (usePatternTexture) {
						outline += visibilityFactor * (1.0 - maskColor.r) * (1.0 - patternColor.r);
					}
		
					// add outline to base; clamp to avoid HDR blowouts when tone mapping later
					gl_FragColor = clamp(baseColor + outline, 0.0, 1.0);
				}
			`,
			blending: NoBlending,
			depthTest: false,
	...
	render( renderer, writeBuffer, readBuffer, deltaTime, maskActive ) {
		...
		if ( this.selectedObjects.length > 0 ) {
			...
			// Combine base (readBuffer) + computed edge textures in shader (no FBO blending)
			this._fsQuad.material = this.materialCombine;
			this.combineUniforms['tDiffuse'].value       = readBuffer.texture;
			this.combineUniforms['maskTexture'].value    = this.renderTargetMaskBuffer.texture;
			this.combineUniforms['edgeTexture1'].value   = this.renderTargetEdgeBuffer1.texture;
			this.combineUniforms['edgeTexture2'].value   = this.renderTargetEdgeBuffer2.texture;
			this.combineUniforms['patternTexture'].value = this.patternTexture;
			this.combineUniforms['edgeStrength'].value   = this.edgeStrength;
			this.combineUniforms['edgeGlow'].value       = this.edgeGlow;
			this.combineUniforms['usePatternTexture'].value = this.usePatternTexture;
			
			// If this pass is final, draw to screen; otherwise write to writeBuffer for the swap.
