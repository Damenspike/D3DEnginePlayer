...
function initRenderer() {
	...
	_editor.updateInspector = updateInspector;
	return renderer;
}
...
function updateInspector(updateAll = false) {
	function bindInputField(element, value, onChange, primaryEvent = 'input change') {
		element.val(value);
	...
	if(selectedObject !== gui.selectedObject || updateAll) {
		...
		if(selectedObject) {
			...
			bindInputField(
				...
				(val, element) => {
					const deg = THREE.MathUtils.degToRad(Number(val) || 0);
					selectedObject.rotation.x = deg;
				}
			);
			bindInputField(
				$("#insp-object-rot-y"), 
			...
			// Draw components
			const componentsElem = $("#insp-components");
			componentsElem.html('');
			selectedObject.components.forEach(component => {
				const schema = D3DComponents[component.type];
				
				if(!schema)
					return;
				
				const jq_component = $("<div class='inspector-cell component-cell no-select'></div>");
				const jq_fields = [];
				...
				for(let i in component.properties) {
					...
					const j_field = $("<div class='field'></div>");
					
					j_field.append($(`<label>${propSchema.label}</label>`));
				...
				jq_component.append(
					$(`<div class='insp-title'>${component.type}</div>`),
					...jq_fields
				);
				componentsElem.append(jq_component);
			});
		}else{
